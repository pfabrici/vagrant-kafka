...
Vagrant.configure("2") do |config|
  ...
  (1..3).each do |i|
    config.vm.define "zookeeper#{i}" do |s|
      ...
      s.vm.network "private_network", ip: "10.30.3.#{i+1}", netmask: "255.255.255.0", virtualbox__intnet: "replic-network", drop_nat_interface_default_route: true
      ...
      s.vm.provision "shell" do |cmd|
        cmd.inline = "su -l -c 'cp /vagrant/config/server${1}.properties /opt/kafka/config && kafka-server-start.sh -daemon /opt/kafka/config/server${1}.properties' kafka"
        cmd.args   = ["#{i}"]
      end
